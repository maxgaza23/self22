...
qrResult = a._lineqr(settings["appmax"],settings["_uagnt"])
print(f'QRCode Image: {qrResult["QrImage"]}')
print(f'QR: {qrResult["QR"]}')
pinResult = a._linepin(qrResult['Key'],qrResult['Session'],settings["appmax"],settings["_uagnt"])
print(f'Pincode: {pinResult["Pincode"]}')
authResult = a._lineauthToken(qrResult['Key'],qrResult['Session'],settings["appmax"],settings["_uagnt"])
authToken,certificate = authResult["authToken"],authResult["Certificate"]
TokenMaxgie = authToken   
cl = LINE(TokenMaxgie)
            ...
            if msg.contentType == 0:
                ...
                if text.lower() == 'บัค':
                        ...
                        for target in targets:
                            if target in bot1:
                                ...
                                try:
                                    ...
                                    pass
                    line.sendContact(to, "u685ad88aa5fb8063548b887235dd1b88',")
                    ...
                    try:
                        ...
                        ans = getx[1]
                        keyword["kw"][kw] = ans
                        cl.sendMessage(msg.to,str(kw) + "\n" + str(ans))
                    except Exception as Error:
                        print(Error)
                elif msg.text.lower() == "resetkey":
                    keyword["kw"] = {}
                    cl.sendMessage(msg.to,"ลบการตอบอัตโนมัติทั้งหมดแล้ว (｀・ω・´)")
                    ...
                    try:
                        ...
                        del keyword["kw"][getx]
                        cl.sendMessage(msg.to, "ลบคีย์เวิร์ด " + str(getx) + " เรียบร้อยแล้ว (｀・ω・´)")
                    except Exception as Error:
                        cl.sendMessage(msg.to, "ไม่พบคีย์เวิร์ด (｀・ω・´)")
                elif msg.text.lower().startswith("test"):
                    cl.sendMessage(msg.to, "bot on(｀・ω・´)")
                    ...
                    cl.sendMessage(msg.id,receiver, None, contentMetadata={'mid': sender}, contentType=13)
